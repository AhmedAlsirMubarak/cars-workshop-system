    ...
    <form method="GET" action="{{ route('inventory.index') }}" class="flex flex-wrap items-center gap-3 mb-6">
        <div x-data="searchBox()">
            <input type="text" name="search" x-model="query" @input="debounce()"
                placeholder="Search SKU or name…" class="input w-56"
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>Name</th>
                        <th>Category</th>
                        <th class="text-end">Cost (OMR)</th>
                        <th class="text-end">Selling (OMR)</th>
                ...
                <tbody>
                    ...
                    <tr class="{{ $isLow ? 'bg-amber-50/50' : '' }}">
                        ...
                        <td>
                            <div class="flex items-center gap-2" x-data="{ editOpen: false }">
                                <button @click="editOpen = !editOpen" class="text-xs text-gray-400 hover:text-orange-500 transition">Edit</button>
                                <button type="button"
                                ...
                                <div x-show="editOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4"
                                    ...
                                    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" @click.stop>
                                        <h3 class="font-semibold text-gray-900 mb-4">Edit — {{ $part->name }}</h3>
                                        <form method="POST" action="{{ route('inventory.update', $part) }}" class="space-y-3">
                                            ...
                                            <div class="grid grid-cols-2 gap-3">
                                                ...
                                                <div>
                                                    <label class="block text-xs font-medium text-gray-600 mb-1">Cost (OMR)</label>
                                                    <input type="number" name="cost_price" value="{{ $part->cost_price }}" step="0.001" min="0" required class="input w-full text-sm">
                                                </div>
                                                <div>
                                                    <label class="block text-xs font-medium text-gray-600 mb-1">Selling (OMR)</label>
                                            ...
                                            <div class="flex gap-3 pt-2">
                                                ...
                                                <button type="button" @click="editOpen = false" class="btn-secondary flex-1">Cancel</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="py-12 text-center text-sm text-gray-400">No parts found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="sm:hidden divide-y divide-gray-50">
            ...
            <div class="p-4 {{ $isLow ? 'bg-amber-50/50' : '' }}">
                <div class="flex justify-between gap-3 mb-1">
                    ...
                    <div class="text-end">
                        <p class="font-bold {{ $isLow ? 'text-red-600' : 'text-gray-900' }}">{{ $part->quantity_in_stock }} in stock</p>
            ...
            @empty
            <p class="py-12 text-center text-sm text-gray-400">No parts found.</p>
            @endforelse
        </div>
    </div>
    ...
    <div x-data="{ open: false }"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 overflow-y-auto max-h-[90vh]"
            ...
            <div class="flex items-center justify-between mb-5">
                <h3 class="font-semibold text-gray-900">Add Part / Component</h3>
            ...
            <form method="POST" action="{{ route('inventory.store') }}" class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required class="input w-full text-sm" placeholder="Part name">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Category</label>
                        <input type="text" name="category" class="input w-full text-sm" placeholder="e.g. Brakes">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Brand</label>
                        <input type="text" name="brand" class="input w-full text-sm" placeholder="Brand name">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Cost Price (OMR) <span class="text-red-500">*</span></label>
                        <input type="number" name="cost_price" required step="0.001" min="0" class="input w-full text-sm" placeholder="0.000">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Selling Price (OMR) <span class="text-red-500">*</span></label>
                        <input type="number" name="selling_price" required step="0.001" min="0" class="input w-full text-sm" placeholder="0.000">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Stock Qty <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        ...
                        <input type="text" name="location" class="input w-full text-sm" placeholder="Shelf / bin">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">Supplier</label>
                        <input type="text" name="supplier" class="input w-full text-sm" placeholder="Supplier name">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Description</label>
                    <textarea name="description" rows="2" class="input w-full text-sm" placeholder="Optional notes…"></textarea>
                </div>
                <div class="flex gap-3 pt-2">
